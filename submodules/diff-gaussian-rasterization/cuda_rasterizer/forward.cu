	...
	__half* __restrict__ out_feat)
{
	...
	int rendered_gaussian_counter = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rendered_gaussian_counter < NUM_FEAT_GAUSSIAN_LIMIT) {
				out_gaussian_idx[pix_id * NUM_FEAT_GAUSSIAN_LIMIT + rendered_gaussian_counter] = collected_id[j];
				out_alpha[pix_id * NUM_FEAT_GAUSSIAN_LIMIT + rendered_gaussian_counter] = acc_constant;
			}
