...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, low_dim_feat):
    first_iter = 0
    ...
    if FAULTY_CARD_FLAG:
        print("Faulty card flag set, will sleep for 50ms after each iteration to avoid crashing the GPU")

    if low_dim_feat:
        # TODO(roger): automate this layer based on low_dim_feat flag and GT feat dim
        my_feat_decoder = feat_decoder([64, 256, 512, 768]).cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        rendered_feat_bhwc = F.interpolate(rendered_feat.unsqueeze(0), size=gt_feat.shape[1:], mode="bilinear", align_corners=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1_000, 4_000, 7_000, 1_0000, 15_000, 20_000, 25_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[500, 1_000, 2_000, 4_000, 7_000, 10_000, 30_000])
